    ...
    await rest.put(Routes.applicationCommands(env.DISCORD_APPLICATION_ID), {
        body: [
            new SlashCommandBuilder()
                .setName('startmc')
                .setDescription('Start minecraft VM instance'),
        ],
    })
}
...
type StartInstanceInit = Omit<
    pubsub.protos.google.pubsub.v1.PubsubMessage,
    ...
    try {
        ...
        if (current.status !== 'TERMINATED') {
            await sendFollowUpMessage(
                `終了状態ではないため、起動を行いません (${current.status})`
            )
        ...
        if (waitedOperation.status === 'DONE') {
            if (waitedOperation.error?.errors) {
                const messages = waitedOperation.error.errors
                    .map(e => e.message || e.code || 'unknown')
                    .join(', ')

                await sendFollowUpMessage(
                    `起動しようとしましたが、エラーが発生しました (${messages})`
                )
            } else {
